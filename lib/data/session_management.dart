...
class SessionManagement {
  ...
  static void setAppSessionNow() async {
    await _preferences.setInt(
        Constants.appSession, DateTime.now().millisecondsSinceEpoch);
  }
  ...
  static String? getNotificationUri() =>
      _preferences.getString("notification_uri");
  static bool getWebLogin() => _preferences.getBool("web_chat_login") ?? false;
  static bool getNotificationSound() =>
      _preferences.getBool("${Constants.package}notification_sound") ?? true;
  static bool getNotificationPopup() =>
      _preferences.getBool("${Constants.package}notification_popup") ?? false;
  static bool getVibration() =>
      _preferences.getBool("${Constants.package}vibration") ?? false;
  static bool getMuteNotification() =>
      _preferences.getBool("mute_notification") ?? false;
  static String getPin() => _preferences.getString("pin") ?? "";
  static String getChangePinNext() =>
      _preferences.getString("change_pin_next") ?? "";
  ...
  static bool isGoogleTranslationEnable() =>
      _preferences.getBool("TranslateLanguageChecked") ?? false;
  static bool isAutoDownloadEnable() =>
      _preferences.getBool("MediaAutoDownload") ?? false;
  static String getTranslationLanguage() =>
      _preferences.getString("LanguageName") ?? "English";
  static String getTranslationLanguageCode() =>
      _preferences.getString("LanguageCode") ?? "en";
  static bool isInitialContactSyncDone() =>
      _preferences.getBool("is_initial_contact_sync_done") ?? false;
  static bool isContactSyncDone() =>
      _preferences.getBool("is_contact_sync_done") ?? false;
  static bool isTrailLicence() =>
      _preferences.getBool("IS_TRIAL_LICENSE") ?? true;
  static int appLastSession() =>
      _preferences.getInt(Constants.appSession) ??
      DateTime.now().millisecondsSinceEpoch;
  static int lastPinChangedAt() =>
      _preferences.getInt(Constants.changedPinAt) ??
      DateTime.now().millisecondsSinceEpoch;
